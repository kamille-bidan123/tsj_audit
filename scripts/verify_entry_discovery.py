...
def fake_entry() -> FunctionInfo:
    return FunctionInfo(
        func_name="add_user",
        file_path="src/web.c",
        start_line=10,
        end_line=30,
        code_snippet='10: int add_user(struct mg_connection *conn) { return 0; }',
...
def verify_entry_discovery_runner_returns_function_info_list() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if entries != [fake_entry()]:
            raise AssertionError("entry discovery should return a FunctionInfo list")
...
def verify_main_saves_discovered_function_info_json() -> None:
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if data[0]["func_name"] != "add_user" or data[0]["skill"] != "civetweb_audit":
            raise AssertionError("discovered_functions.json should contain discovered FunctionInfo entries")
...
if __name__ == "__main__":
    verify_entry_discovery_runner_returns_function_info_list()
    verify_main_saves_discovered_function_info_json()
