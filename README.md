...
- `permission.read`、`permission.glob`、`permission.grep`、`permission.list` 允许只读源码操作。
- `permission.bash` 允许读源码、搜索和运行内置扫描脚本，其它 bash 继续 ask。
- `permission.external_directory` 允许越过当前工作目录访问外部路径，避免只读审计时反复确认目录边界。
...
- 右侧函数列表可点击切换日志视图；默认显示全局日志，点击函数后显示该函数对应的 OpenCode session/tool 日志。
...
- `checkpoints/`：断点续审数据。
- `checkpoints/logs/`：按 `file_path:start_line:func_name` 归属保存的函数级 runtime 日志，文件名会将路径分隔符等字符替换为 `_`，格式为 JSONL。
...
继承 SARIF Explorer 人工审核结果：

```bash
go run ./cmd/export-results --output-dir output --inherit-sarif old_reviewed.sarif
```

该选项会读取 `old_reviewed.sarif.sarifexplorer` 中的 `status/comment`，按 SARIF result 的稳定指纹或兼容字段匹配到新导出的结果，并生成新的 `audit_report.sarif.sarifexplorer` / `audit_issues.sarif.sarifexplorer`。
