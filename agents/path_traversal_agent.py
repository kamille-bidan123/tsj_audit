...
class PathTraversalSubmitTool:
    """提交路径遍历审计结果的工具"""

    name = "submit_path_traversal"
    description = "提交路径遍历审计结果"

    commands = {
        "submit_path_traversal": {
            "description": "路径遍历审计完成，提交审计结果",
            "parameters": {
                "type": "object",
                "properties": {
                    "summary": {
...
# 注册 submit_path_traversal 工具
ToolRegistry._commands["submit_path_traversal"] = PathTraversalSubmitTool
ToolRegistry._tools["submit_path_traversal"] = PathTraversalSubmitTool
...
class PathTraversalAgent:
    ...
    def _save_conversation_history(self, messages: List[Dict]):
        ...
        conversations_dir = Path(self.output_dir) / "conversations" / "path_traversal_agent"
        ...
        if self.debug:
            print(f"  [对话历史] path_traversal_agent 已保存: {conversation_file}", file=sys.stderr)
        ...
        for turn in range(max_turns):
            ...
            if response.tool_calls:
                for tc in response.tool_calls:
                    ...
                    # 检查是否是 submit_path_traversal 调用
                    if func_name == "submit_path_traversal":
                        if self.debug:
                    ...
                    # 特殊处理，禁止直接search_code搜索危险函数
                    if func_name == "search_code" and "pattern" in arguments and arguments["pattern"] in self.DANGEROUS_FUNCTIONS:
                        print(f"  [警告] 禁止直接搜索危险函数 '{arguments['pattern']}'，请通过分析数据流来发现潜在的路径穿越漏洞", file=sys.stderr)
                        result = f"错误：禁止直接搜索危险函数 '{arguments['pattern']}'，请通过分析数据流来发现潜在的路径穿越漏洞"
                    # 执行工具
                    else:
                ...
                messages.append({
                    "role": "user",
                    "content": "请使用工具进行审计，或调用 submit_path_traversal 工具结束审计。"
                })
