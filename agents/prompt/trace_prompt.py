...
## 审计目标
1. 使用 EntrySpec/FunctionInfo.skill 指定的知识库识别外部输入点
2. 追踪数据如何传递给其他函数
...
## Output Format
当你认为已经探索完成时，直接按结构化输出返回 function_info、code_logic 和 code_map。
"""


def build_exploration_user_message(_func_info=None) -> str:
    """构建探索阶段用户消息"""
    return """请根据 system prompt 中的 EntrySpec/FunctionInfo、Function Skill 和源码上下文进行 trace。
